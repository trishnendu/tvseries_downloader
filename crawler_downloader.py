...
def download(url, display_hook = None):
    ...
    while(True):
        ...
        if not(chunk):
            break
        notify(bytes_so_far, total_size)
    ...
    if total_size != bytes_so_far:
        ...
        exit(1)
    sys.stdout.write("Download complete :) -> "+os.path.join(os.getcwd(), outfilename)+"\n" )
    outfp.close()
    return
...
crawler("http://fastdownloadhub.com/Serial/Better Call Saul/", crawl_depth = 1, download_content = True)